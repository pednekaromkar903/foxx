...
import {
  ...
  Code2, Table2, Sparkles, CheckCircle2, Circle
} from 'lucide-react';
import {
...
export function IntelligenceChat({ mode = 'full', onClose }: IntelligenceChatProps) {
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        {
          ...
          followUps: d.followUps || d.suggestedActions,
          suggestedActions: d.suggestedActions || d.followUps,
          steps: d.steps,
          intent: d.intent,
        },
      ]);
      ...
      setMessages((m) => [
        ...m,
        { role: 'ai', content: 'Unable to reach the analytics engine. Ensure the database is connected and try again.' },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const containerClass = isWidget
    ...
    : 'flex h-[calc(100vh-7rem)] flex-col rounded-2xl overflow-hidden shadow-atomberg-lg border border-atomberg-gold/20 bg-white';
